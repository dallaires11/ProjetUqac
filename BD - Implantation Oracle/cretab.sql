...
numAbonnement INTEGER,
FOREIGN KEY (numAbonnement) REFERENCES Abonnement(NUMABONNEMNT),
dateDebut DATE,
...
CONSTRAINT checkIdNumEmp CHECK (numEmploye<100000000 AND numEmploye>9999999),
personid INTEGER,
...
FOREIGN KEY (personid) REFERENCES Personne(personid),
mdp VARCHAR(25) NOT NULL,
CONSTRAINT checklongueurmdp CHECK (LENGTHB(mdp) >=5),
...
CREATE VIEW ClientView AS
   SELECT p.personid,p.prenom,p.nom,p.courriel,c.mdp,c.numclient,c.cartecredit,c.dateexpiration,p.adresse,p.datenaissance
   FROM personne p ,client c
   WHERE p.personid=c.personid;

CREATE VIEW EmployeViewView AS
   SELECT p.personid,e.numemploye,p.prenom,p.nom,e.identifiant,e.mdp,p.courriel,p.adresse,p.datenaissance
   FROM personne p ,employe e
